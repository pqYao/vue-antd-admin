<template>
  <a-card>
    ...
    <div class="notice-preview" id="printMe">
      <h2 style="text-align:center">{{ baseData.noticeTitle }}</h2>
      <div v-html="baseData.noticeContext"></div>
      <div style="text-align:right" v-if="baseData.entrustFlag == 0">
        招标人：{{ baseData.projectCreateUnitName }}
      </div>
      <div style="text-align:right" v-else>
        招标人/招标代理机构：{{
          baseData.projectCreateUnitName + "/" + baseData.agencyOrgName
        }}
      </div>
      <div style="text-align:right">
        {{
          baseData.operationFinishDate ||
...
export default {
  ...
  methods: {
    moment,
    getData() {
      findSendTenderNoticeByProjectId(
