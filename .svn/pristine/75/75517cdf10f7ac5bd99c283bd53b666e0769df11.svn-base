    ...
    <a-card>
      ...
      >
        ...
        <template slot="purchaseType" slot-scope="text">
          {{ text ? dictionary.comparatePurchaseType[text] : "" }}
        </template>
        <!-- 是否委托 -->
...
import { getNoticeChangeList } from "@/services/compare";
...
export default {
  ...
  methods: {
    ...
    handleAction(record, type) {
      if (type == 0) {
        this.$router.push({
          path: `/compare/tender/notice-change-edit/${record.projectId}`,
          query: {
            ncid: "",
      ...
      if (type == 2) {
        this.$router.push({
          path: `/compare/tender/notice-change-detail/${record.projectId}`,
          query: {
            ncid: record.noticeChangeId,
            projectId: record.projectId
          }
    ...
    fnJumpPage(record) {
      this.$router.push({
        path: `/compare/prepare/detail/${record.projectId}`
      });
    }
  }
};
