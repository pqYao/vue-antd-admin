    ...
    <a-card>
      ...
      >
        ...
        <template slot="action" slot-scope="text, record">
          ...
          <!-- v-if="record.payMode == 2 && record.operationState == 401" -->
          <a style="margin-right: 8px" @click="handleAction(record, 4)">
            下载付款凭证
          </a>
          <!-- 已确认才展示下载采购文件 -->
          <!-- v-if="record.operationState == 403" -->
          <a style="margin-right: 8px" @click="handleAction(record, 3)">
            下载采购文件
          </a>
        </template>
      </a-table>
...
import { toConfirmFilePay } from "@/services/bid";
export default {
  name: "purchase-files",
  components: {
    ...
    handleAction(data, type) {
      ...
      switch (type) {
        ...
        case 2:
          ...
          toConfirmFilePay(param).then(res => {
            const resData = res.data || {};
            if (resData.errCode == "0000") {
