    ...
    <a-card>
      ...
      >
        ...
        <template slot="purchaseType" slot-scope="text">
          {{
            dictionary.comparatePurchaseType
              ? dictionary.comparatePurchaseType[text]
              : ""
          }}
        </template>
        <!-- 是否委托 -->
...
import { findSubContractForPage } from "@/services/compare";
...
export default {
  ...
  methods: {
    ...
    handleEdit(id) {
      this.$router.push({
        path: `/compare/prepare/sub-contract-edit/${id}`
      });
    },
    projectDetail(id) {
      this.$router.push({
        path: `/compare/prepare/detail/${id}`
      });
    },
    handleDetail(id) {
      this.$router.push({
        path: `/compare/prepare/sub-contract-detail/${id}`
      });
    },
