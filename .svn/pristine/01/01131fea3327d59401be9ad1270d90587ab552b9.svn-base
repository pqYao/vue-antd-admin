...
import { findSendTenderInfoByPage } from "@/services/bid";
...
export default {
  ...
  methods: {
    ...
    handleEdit(record) {
      this.$router.push({
        path: `/bid/tender/tender-info-edit/${record.tenderVindicateId}`,
        query: {
    ...
    handleDetail(record) {
      this.$router.push({
        path: `/bid/tender/tender-info-detail/${record.tenderVindicateId}`,
        query: {
          purchaseType: record.purchaseType,
          projectId: record.projectId
        }
