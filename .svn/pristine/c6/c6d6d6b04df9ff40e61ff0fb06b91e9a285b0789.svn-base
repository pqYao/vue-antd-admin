...
import { findSendTenderInfoByPage } from "@/services/compare";
...
export default {
  ...
  methods: {
    ...
    handleEdit(record) {
      this.$router.push({
        path: `/compare/tender/tender-info-edit/${record.tenderVindicateId}`,
        query: {
    ...
    handleDetail(record) {
      this.$router.push({
        path: `/compare/tender/tender-info-detail/${record.tenderVindicateId}`,
        query: {
          purchaseType: record.purchaseType,
          projectId: record.projectId
        }
