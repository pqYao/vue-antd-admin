    ...
    <a-card>
      ...
      >
        ...
        <template slot="purchaseType" slot-scope="text">
          {{
            dictionary.purchaseTypeAll ? dictionary.purchaseTypeAll[text] : ""
          }}
        </template>
        <!-- 是否委托 -->
        ...
        <template slot="action" slot-scope="record">
          <!-- 300: "待应答";301: "应答中";302: "已应答";303: "已过期" -->
          <!-- v-if="record.operationState == 300 || record.operationState == 301" -->
          <a style="margin-right: 8px" @click="handleEdit(record.projectId)">
            编辑
          </a>
          <a
            v-if="record.operationState == 302 || record.operationState == 303"
...
import { getAnswerList } from "@/services/response";
