...
import { findProjectInfoById, findProjectExplain } from "@/services/other";
...
export default {
  ...
  data() {
    return {
      ...
      baseList: [
        ...
        { label: "单一来源场景", key: "singleSourceScene", show: false },
  ...
  methods: {
    ...
    initList(data) {
      if (data.purchaseType == "6" || data.purchaseType == "8") {
        this.explainlist[0].show = true;
      }
      if (data.purchaseType == "5" || data.purchaseType == "7") {
        this.baseList[10].show = true;
      }
      if (data.purchaseType === "9") {
        this.baseList[4].show = true;
        this.explainlist[1].show = true;
      }
      if (data.entrustFlag === "1") {
        this.baseList[12].show = true;
        this.baseList[13].show = true;
      }
    }
  }
};
