<template>
	<div class="file-table-wrapper">
		<a-card>
      <Heading :name="tableName">
      	<template v-slot:leftIcon>
      		<a-popover placement="bottom">
						<template slot="content">
				      <p>系统支持mht/rar/zip格式的附件上传，最大不能超30M。</p>
				    </template>
      	...
      	<div v-if="pageType == 1">
	        ...
	        <a-button type="primary" @click="fnDelete">
...
<script>
	...
	export default {
		...
		methods: {
	      ...
	      // let url = `${this.BASE.apiUrl.file}/fastDfsFileManage/fastdfs/uploadFile`
	      // uploadForm.append('upload', this.fileList[0])
	      // for (let i in param) {
	      //   uploadForm.append(i, param[i])
	      // }
	      // this.$http.postForm(url, uploadForm).then(res => {
	      //   if (res.errCode == '0000') {
	      //     this.$message.success('文件上传成功！')
	      //     //再次检测必传的文件类型是否上传
	      //     this.fnJudgeRequiredFileType()
	      //     //新增成功时时文件类型列变窄
	      //     this.columns[1].width = this.waitSource.length == 0 ? 140 : 300
	      //     this.fnGetData()
	      //     // this.fileList = [];
	      //   } else {
	      //     this.$message.warning(
	      //       _self.BASE.handleErrorMsg(res) || '文件上传失败!'
	      //     )
	      //   }
	      // })
			},
