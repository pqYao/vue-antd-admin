<template>
  <div class="bs-sign-up-wrapper">
    <SearchForm
      ref="SearchForm"
      name="查询条件"
      stateCode="innerEnroll"
    ...
    <a-card>
      ...
      >
        ...
        <template slot="action" slot-scope="record">
          <!-- 
            当前登录人为单据的创建人
            业务状态为‘报名中’展示编辑、否则展示查询
          -->
          <a
            v-if="record.operationState == 600"
          ...
          <a
            v-if="record.operationState == 601"
            style="margin-right: 8px"
...
export default {
  ...
  methods: {
    fnGetData() {
      ...
      getBidSubmitSignUpList(param).then(res => {
    ...
    getOperationStateList() {
      operationStateMap("innerEnroll").then(result => {
    ...
    handleEdit(id) {
      this.$router.push({
        path: `/bid/submit/bid-submit-sign-up-edit/${id}`
      });
    },
    handleDetail(id) {
      this.$router.push({
        path: `/bid/submit/bid-submit-sign-up-detail/${id}`
      });
    },
    handleSearch(data) {
      this.filter = data;
