<template>
  <div class="dt-product-info-wrapper">
    <a-card>
      <Heading name="产品明细">
        ...
        <div
          ...
          v-if="pageType == 1 && divideTenderDataLength != 0"
        >
          <a-button type="primary" @click="fnAddProduct">
            关联产品
          </a-button>
          <a-popconfirm
    ...
    >
      <ProductSelect ref="productSele" :param="seleProParam"> </ProductSelect>
    </a-modal>
  </div>
</template>
  ...
  methods: {
    ...
    fnHandleChange(value, key, column) {
      ...
      if (target) {
        ...
        this.dataSource.forEach(ele => {
          if (
            target.productId == ele.productId &&
            ele.subtenderDetailId != target.subtenderDetailId
          ) {
