  ...
  <div>
    <a-card>
      ...
      <a-table
        ...
        :scroll="{ x: 1000 }"
      >
        <template slot="requirementAmout" slot-scope="text, record, index">
...
export default {
  ...
  methods: {
    ...
    checkAmount(rule, value, callback) {
      ...
      if (!reg.test(value)) {
        callback("请输入0.01-999999999999.99的数值！");
      }
      callback();
    },
    ...
    // 删除
    deleteById(id) {
      let projectDetailIds = id || this.selectedRowKeys.join(",");
      deleteProductInfo(projectDetailIds).then(res => {
        const resData = res.data;
        if (resData.errCode === "0000") {
          const { responseResult } = resData;
