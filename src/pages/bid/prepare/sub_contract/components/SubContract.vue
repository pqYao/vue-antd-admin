            ...
            <a-form-item>
              <a-input
                ...
                v-decorator="[
                  `bondMoney${index}`,
                  {
                    rules: [{ required: true, validator: validMoneyHasZero }],
                    validateTrigger: ['blur'],
                    initialValue: text
                  }
                ]"
...
export default {
  ...
  methods: {
    ...
    validMoneyHasZero(rule, value, callback) {
      let reg = this.BASE.reg.moneyRegHasZero;
      if (!this.BASE.CheckIsNullOrEmpty(value)) {
        callback("请输入金额");
        return;
      }
