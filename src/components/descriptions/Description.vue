<template>
  <a-descriptions bordered size="small" :column="{ md: 3, sm: 2, xs: 1 }">
    ...
    >
      ...
      <template v-else-if="item.dic">
       ...
       <template v-else-if="item.pt">
        {{ dictionary[item.key][param['purchaseType']] }}
      </template>
      <template v-else-if="item.jump">
        <a @click="fnJumpPage(item.key, jumpData)">{{ param[item.key] }}</a>
      </template>
      <template v-else-if="item.tinymce">
        <div v-html="param[item.key]"></div>
      </template>
      <template v-else>
        {{ param[item.key] }}
      </template>
...
/deep/.ant-descriptions-row:nth-of-type(1) {
  visibility: collapse;
}
